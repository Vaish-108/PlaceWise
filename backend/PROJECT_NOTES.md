...
"What is the most unique part of your project?"

You can say:

"I built an AI Placement Mentor that combines resume analysis, AI-powered placement readiness, job matching, personalized skill-gap analysis, career roadmaps, and an intelligent chat assistant into one unified dashboard. Instead of using AI as a separate chatbot, it understands the student's profile and provides personalized placement guidance."

That answer is far more memorable than saying, "I integrated an AI chatbot."
...
"How can I improve my chances for Microsoft?"

the backend should:

Identify the logged-in student from the JWT.
Load the student's profile from MongoDB.
Load the uploaded resume (if available).
Load extracted skills.
Load AI suggestions.
Build one intelligent prompt.
Send that prompt to generateResponse().
Return only Gemini's response.
